...
class GTExTable(AnyTable):
        ...
        def dict_create(self):
                ...
                При создании словаря ключи автоматически сортируются, что пригодится для создания довольно быстрого алгоритма.
...
class RefSNPIDTable(AnyTable):
        ...
        def rs_search(self, rs_and_ann_dict):
                ...
                Поскольку искомые refSNPID и GTEx-refSNPID-ключи были отсортированы идентичными способами,
                можно будет оптимизировать скорость поиска.
                При работе с текущим набором искомых refSNPID будет создаваться новый сокращающийся GTEx-словарь.
                Сокращаться словарь будет следующим образом:
                Вначале будут удалены элементы, находящиеся вне диапазона от минимального до максимального из искомых refSNPID текущего файла.
