...
def rs_col_index_search(two_dim):
        ...
        Столбец с refSNPID может быть единственным в подаваемом на вход двумерном массиве.
        ...
        for row_num in range(1, len(two_dim)):
                for cell in two_dim[row_num]:
                        if re.match(r'rs\d+', cell):
                                return two_dim[row_num].index(cell)
...
class GTExTable(AnyTable):
        ...
        def clean_and_sort(self):
                ...
                for row in self.two_dim:
                        if re.search(r'rs\d+', row[self.rs_col_index]) != None:
                                two_dim_pure.append(row)
                two_dim_pure.sort(key = lambda row: row[self.rs_col_index])
                return two_dim_pure

        def dict_create(self):
                '''
                Создание словаря, в котором ключи - refSNPID, а значения - соответствующие им строки таблицы.
                ...
                Если же refSNPID уникален, то соответствующая ему строка таблицы тоже пойдёт в двумерный массив,
                служащий значением refSNPID-ключу, но содержащий лишь один вложенный список.
                '''
...
class RefSNPIDTable(AnyTable):
        '''
        Действия, производимые с refSNPID-содержащими таблицами.
        '''
        def __init__(self, two_dim, rs_col_index, rs_and_ann_dict):
                super().__init__(two_dim, rs_col_index)
                self.rs_and_ann_dict = rs_and_ann_dict
        ...
        def rs_search(self):
                ...
                for row in self.two_dim:
                        ...
                        if rs in self.rs_and_ann_dict:
                                out_two_dim += self.rs_and_ann_dict[rs]
                                del self.rs_and_ann_dict[rs]
                return out_two_dim
...
#Удаление строк GTEx-таблицы, содержащих точку вместо refSNPID, и сортировка очищенной GTEx-таблицы.
gtex_table = GTExTable(gtex_two_dim, gtex_rs_col_index)
gtex_two_dim_pure = gtex_table.clean_and_sort()
...
for rs_file in rs_files:
        ...
        refsnpid_table = RefSNPIDTable(rs_table_pure, rs_rs_col_index, gtex_dict)
        found_in_gtex = sorted(refsnpid_table.rs_search(), key = lambda row: row[gtex_rs_col_index])
