...
col_numbers = [int(col_number) for col_number in input('''Номер одного или номера нескольких экстрагируемых столбцов
(через пробел): ''').split()]
num_of_headers = int(input('''Количество не считываемых строк в начале файла
(совет: табулированную шапку к ним не причисляйте) [0|1|2|(...)]: '''))
...
for src_file_name in src_file_names:
        with open(os.path.join(src_dir_path, src_file_name)) as src_file_opened:
                ...
                #Конструируемое ниже имя конечного файла будет содержать номера выделенных столбцов.
                ...
                with open(os.path.join(trg_dir_path, trg_file_name), 'w') as trg_file_opened:
                        ...
                        for line in src_file_opened:
                                ...
                                #Формирование списка, содержащего "ячейки" только запрашиваемых "столбцов".
