...
не страдает утечками памяти, не требует установки и запускается почти на любой ОС.
                ...
                while switching_flag:
                        ...
                        for row_1 in self.two_dim_1[self.two_dim_1_row_num:]:
                                ...
                                for row_2 in self.two_dim_2[self.two_dim_2_row_num:]:
                                        ...
                                        ##Пытаемся пересечь.
                                        ##Случаи, когда chr 1-го и 2-го списка не равны друг другу,
                                        ##уже были обработаны ранее, поэтому на этапе непосредственно
                                        ##пересечений сравнивать хромосомы не нужно - они точно равны.

                                        #-----(s1---[{s2-----e1})---e2]-----
                                        #1-й интервал на этом этапе уже не может быть левее 2-го,
                                        #т.к. этот случай должен был быть обработан ранее.
                                        #Это позволяет избежать явного указания
                                        #невавенства между start_2 и end_1.
                                        if start_1 <= start_2 and end_1 <= end_2:
                                                intersection = [chrom_1, start_2, end_1]
                                                success += 1

                                        #-----[s2---({s1-----e2}]---e1)-----
                                        #Здесь указание неравенства start_1 с end_2 обязательно,
                                        #т.к. случай с уползанием 1-го интервала направо будет обработан
                                        #лишь после осуществления попыток пересечения (см. ниже (**)).
                                        elif start_2 <= start_1 < end_2 <= end_1:
                                                intersection = [chrom_1, start_1, end_2]
                                                success += 1

                                        #-----(s1---[{s2-----e2}]---e1)-----
                                        #В этом случае указание неравенства между start_2 и end_2
                                        #не трубуется, т.к. в пересекаемые двумерные
                                        #массивы не попадают интервалы с нулевой дельтой.
                                        elif start_1 <= start_2 and end_2 <= end_1:
                                                intersection = [chrom_1, start_2, end_2]
                                                success += 1

                                        #-----[s2---({s1-----e1})---e2]-----
                                        #Поскольку равные друг другу start_1 и end_1 не могут
                                        #поступить в пересекаемые наборы данных, то явно
                                        #указывать неравенство между ними не обязательно.
                                        elif start_2 <= start_1 and end_1 <= end_2:
                                                intersection = [chrom_1, start_1, end_1]
                                                success += 1

                                        ####################

                                        #-----[s2-----e2]---(s1-----e1)]-----
                                        #(**)Если на данном этапе не пересеклось, то получается,
                                        #что интервал из 1-го списка находится правее.
                                        #Этот блок кода сработает только, если ранее не было пересечений.
                                        #Меняем местами списки интервалов без сдвига указателя 1-го.
                                        else:
                                                switching_flag = False
                                                break
                                        ...
                                        #(*)Если успешное пересечение было с последним интервалом
                                        #второго списка, то нужно сразу переключить списки.
                                        #Сдвигаем указатель на следующий интервал 1-го списка.
